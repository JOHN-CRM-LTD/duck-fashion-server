...
mkdir -p .local-duck
node - "$1" "$read_key" "$write_key" "$PWD/data" <<'EOF'
const fs = require("fs");
const [url, apiKey, writeApiKey, dataDirectory] = process.argv.slice(2);
fs.writeFileSync(".local-duck/live-connection.json",
  JSON.stringify({ mode: "capsule", port: 4997, apiKey, writeApiKey, dataDirectory, url }, null, 2) + "\n");
EOF
